 ...
 channel.setParent(message.guild.channels.find(channel => channel.name === "✮ ▬  ▬ Duyuru Kanalları▬  ▬ ✮")));
  ...
  .then(c => {
    ...
    c.overwritePermissions(role, {
       ...
       message.guild.owner.send("Gerekli Herşey Kuruldu Rahatına Bak! **XRY  Bot'un kıymetini bil :D**")
...
if (!modlogkanal) return;
	if (channel.type === "text") {
				...
				.setFooter(`,XRY-BOT | Mod-Log Sistemi Kanal ID: ${channel.id}`)
...
.setTitle("SES KANALI OLUŞTURULDU")
				...
				.setFooter(`XRY | Mod-Log Sistemi Kanal ID: ${channel.id}`)
